...
%token <string> JSX_HTML_TAG  "<html-tag" (* just an example *)
%token <string> JSX_HTML_TAG_E "</html-tag" (* just an example *)
%token <string> JSX_ATTR      "html-attr" (* just an example *)
%token LPAREN                 "("
%token LBRACKETAT             "[@"
%token LBRACKETATAT           "[@@"
...
;
jsx_element:
    tag=jsx_longident(JSX_UIDENT, JSX_HTML_TAG) props=llist(jsx_prop) SLASHGREATER {
      let children =
        let children, loc = mktailexp $loc(tag) [] in
        mkexp ~loc children
      in
      Jsx_helper.make_jsx_element () ~raise ~loc:$loc(tag) ~tag ~end_tag:None ~props ~children }
  | tag=jsx_longident(JSX_UIDENT, JSX_HTML_TAG) props=llist(jsx_prop) 
    GREATER children=llist(simple_expr) end_tag=jsx_longident(JSX_UIDENT_E, JSX_HTML_TAG_E) end_tag_=GREATER {
      let children = 
