...
let split_html_name name =
  let len = String.length name in
  let rec loop i =
    if i = len then None
    else if name.[i] = '-' || name.[i] = ':' then Some i
    else loop (i + 1)
  in
  loop 0

let rewind_lexbuf lexbuf offset =
  let pos_cnum = lexbuf.Lexing.lex_start_p.Lexing.pos_cnum + offset in
  lexbuf.Lexing.lex_curr_pos <- lexbuf.Lexing.lex_start_pos + offset;
  lexbuf.Lexing.lex_curr_p <- { lexbuf.Lexing.lex_start_p with Lexing.pos_cnum }
