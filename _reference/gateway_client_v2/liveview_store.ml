...
let save ~user_id ~topic ~endpoint ~model_json =
  let db = get_db () in
  let sql = {|
    INSERT OR REPLACE INTO liveview_state (user_id, topic, endpoint, model_json, updated_at)
    VALUES (?, ?, ?, ?, ?)
  |} in
  ...
  let _ = Sqlite3.bind stmt 4 (Sqlite3.Data.TEXT (Yojson.Safe.to_string model_json)) in
...
let load ~user_id ~topic =
  let db = get_db () in
  let sql = "SELECT endpoint, model_json FROM liveview_state WHERE user_id = ? AND topic = ?" in
  let stmt = Sqlite3.prepare db sql in
  let _ = Sqlite3.bind stmt 1 (Sqlite3.Data.TEXT user_id) in
  let _ = Sqlite3.bind stmt 2 (Sqlite3.Data.TEXT topic) in
...
let delete ~user_id ~topic =
  let db = get_db () in
  let sql = "DELETE FROM liveview_state WHERE user_id = ? AND topic = ?" in
  let stmt = Sqlite3.prepare db sql in
  let _ = Sqlite3.bind stmt 1 (Sqlite3.Data.TEXT user_id) in
  let _ = Sqlite3.bind stmt 2 (Sqlite3.Data.TEXT topic) in
  let _ = Sqlite3.step stmt in
...
let cleanup ?(max_age_days = 30) () =
  let db = get_db () in
  let cutoff = Unix.gettimeofday () -. (Float.of_int max_age_days *. 86400.0) in
