...
let to_equal expected exp =
  let equal = exp.value = expected in
  if (equal && exp.negated) || (not equal && not exp.negated) then
    fail_with exp (Printf.sprintf "%s to equal %s"
      (Obj.magic exp.value |> string_of_int) (* Simple fallback *)
...
let to_be_greater_than threshold exp =
  ...
  if (greater && exp.negated) || (not greater && not exp.negated) then
    fail_with exp (Printf.sprintf "%d to be greater than %d" exp.value threshold)
...
let to_be_greater_than_float threshold exp =
  let greater = exp.value > threshold in
  if (greater && exp.negated) || (not greater && not exp.negated) then
    fail_with exp (Printf.sprintf "%f to be greater than %f" exp.value threshold)

let to_be_less_than_float threshold exp =
  let less = exp.value < threshold in
  if (less && exp.negated) || (not less && not exp.negated) then
    fail_with exp (Printf.sprintf "%f to be less than %f" exp.value threshold)

let to_contain substring exp =
  let contains =
    try
      let _ = Str.search_forward (Str.regexp_string substring) exp.value 0 in
      true
    with Not_found -> false
  in
  if (contains && exp.negated) || (not contains && not exp.negated) then
    fail_with exp (Printf.sprintf "\"%s\" to contain \"%s\"" exp.value substring)
...
let to_have_length expected_len exp =
  ...
  if (equal && exp.negated) || (not equal && not exp.negated) then
    fail_with exp (Printf.sprintf "list to have length %d but got %d" expected_len actual_len)

let to_raise _fn exp =
  let raised =
    try
      let _ = exp.value () in
      false
...
let to_raise_with expected_msg exp =
  ...
  | Some msg when exp.negated ->
      fail_with exp (Printf.sprintf "function not to raise, but got: %s" msg)
  | Some msg when msg <> expected_msg && not exp.negated ->
      fail_with exp (Printf.sprintf "exception message \"%s\" to equal \"%s\"" msg expected_msg)
  | _ -> ()
...
let run_test suite test =
  ...
  begin
    ...
    | Assertion_failed msg ->
      test.result <- Some (Fail msg)
    | exn ->
      test.result <- Some (Fail (Printf.sprintf "Unexpected exception: %s" (Printexc.to_string exn)))
  end;
  test.duration_ms <- get_time_ms () -. start_time;

  (* Run after_each hook *)
  (match suite.after_each with
   | Some fn -> fn ()
   | None -> ())

let run_suite ~(filter : string option) ?(ci_mode=false) (suite : test_suite) =
  (* Run before_all hook *)
  (match suite.before_all with
   | Some fn -> fn ()
   | None -> ());

  (* Filter and reverse tests to run in definition order *)
  let tests : test_case list = List.rev suite.tests in
  let tests : test_case list = match filter with
    | None -> tests
    | Some pattern ->
      let re = Str.regexp pattern in
      List.filter (fun (t : test_case) ->
        try let _ = Str.search_forward re t.name 0 in true
        with Not_found -> false
      ) tests
  in
  ...
  (* Print suite header *)
  if not ci_mode then
    print_endline (Color.bold suite.name);
  ...
  List.iter (fun test ->
    ...
    | _ ->
      ...
      (match test.result with
       | Some Pass ->
         if ci_mode then
           Printf.printf "PASS %s > %s (%.2fms)\n" suite.name test.name test.duration_ms
         else
           Printf.printf "  %s %s %s\n"
             (Color.green "✓")
             test.name
...
let run ?(filter=None) ?(ci_mode=false) () =
  let start_time = get_time_ms () in

  (* Reverse suites to run in definition order *)
  ...
  if ci_mode then begin
    ...
    Printf.printf "  %s\n" (Color.gray (Printf.sprintf "Total: %d tests in %.2fms" !total duration_ms))
  end;

  { total = !total; passed = !passed; failed = !failed; skipped = !skipped; duration_ms }
...
let show_diff expected actual =
  ...
  List.iter (fun line -> Printf.printf "  %s\n" (Color.green ("+ " ^ line))) expected_lines;
  print_endline (Color.dim "Actual:");
  List.iter (fun line -> Printf.printf "  %s\n" (Color.red ("- " ^ line))) actual_lines
